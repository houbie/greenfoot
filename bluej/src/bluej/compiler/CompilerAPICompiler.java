...
//            sjfm.close();
            ...
            List<String> cp = new ArrayList<String>(pathList.size());
            for (File file : pathList) {
                cp.add(file.getAbsolutePath());
            }
            config.setClasspathList(cp);
            config.setTargetDirectory(getDestDir());
            ...
            if (e.getErrorCollector() != null) {
                for (Object m : e.getErrorCollector().getErrors()) {
                    Message message = (Message) m;
                    StringWriter sw = new StringWriter();
                    message.write(new PrintWriter(sw));
                    bluej.compiler.Diagnostic diagnostic;
                    if (message instanceof SyntaxErrorMessage) {
                        SyntaxException se = ((SyntaxErrorMessage) message).getCause();
                        diagnostic = new bluej.compiler.Diagnostic(bluej.compiler.Diagnostic.ERROR, sw.toString(),
                                se.getSourceLocator(), se.getStartLine(), se.getStartColumn(), se.getEndLine(), se.getEndColumn());
            ...
            bluej.compiler.Diagnostic diagnostic = new bluej.compiler.Diagnostic(bluej.compiler.Diagnostic.ERROR, e.toString());
            observer.compilerMessage(diagnostic);
        } catch (Throwable e) {
